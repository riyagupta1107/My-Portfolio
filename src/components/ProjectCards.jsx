...
function ProjectCards({ project, isActive }) {
    return (
      <div 
        className={`
          flex-shrink-0 w-72 md:w-[22rem] mx-[-0.9rem] snap-center 
          transition-transform duration-500 ease-in-out relative
          ${isActive ? 'z-20 scale-110' : 'z-10 scale-90 opacity-70'}
        `}
      >
        <div 
          className="bg-white shadow-xl border border-black overflow-hidden"
          style={{
        ...
        >
            ...
            {project.imageUrl && (
              <img
                ...
                alt={project.title}
                className="w-full h-[9rem] object-cover rounded-xl border border-black"
                />
              )}
            </div>
          
          <div className="px-7 text-center">
            <h3 className="text-2xl font-customFont font-semibold mb-2">{project.title}</h3>
            <p className="text-xs font-myFont text-gray-500 mb-3"><strong>Tech Stack:</strong> {project.techStack}</p>
            <p className="text-gray-700 mb-4 font-textFont text-sm">{project.description}</p>
            <a 
              href={project.link} 
              target="_blank" 
